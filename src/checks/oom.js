...
export const oom = defineCheck({
  ...
  async run(ctx) {
    ...
    // Count distinct KILL EVENTS, not log lines: one OOM produces both a
    // "Killed process <pid>" and an "oom_reaper: reaped process <pid>" line,
    // which used to be counted as two kills (title + high severity wrong).
    const pidOf = (l) => {
      const m = l.match(/(?:Killed|reaped) process\s+(\d+)/i) || l.match(/oom-kill:.*\bpid=(\d+)/i);
      return m ? m[1] : null;
    };
    const pids = new Set(uniq.map(pidOf).filter(Boolean));
    const events = pids.size > 0 ? pids.size : uniq.length;

    // Show the most recent kills — the fix is the same regardless, but the
    // count tells the user if it is a one-off or a pattern.
    const sample = uniq.slice(-5).join("\n");
    const isRecent = events >= 2; // multiple kills = pattern = more urgent
    return [finding({
      severity: isRecent ? "high" : "medium",
      code: "oom/kills",
      title: `${events} OOM kill(s) in the recent kernel log`,
      detail: events === 1
        ...
        : `The kernel killed ${events} processes recently to free memory. Repeated OOM kills mean the system is consistently overcommitted — the heaviest app is being sacrificed.`,
      evidence: sample,
